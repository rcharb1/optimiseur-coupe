...
void FenetrePrincipale::traiterFormulaire(){
    ...
    if(erreur==0){
        ...
        if(m_client->sendMessage(message) != 0) {
            QMessageBox(QMessageBox::Warning, tr("Erreur"), tr("Une erreur est apparue lors de l'envoie du message au serveur.\n"
                                                               "Veuillez vérifier la connexion"), QMessageBox::Ok);
        }
        qDebug() << "Fenêtre Principale : j'attends de recevoir un message...";
        while(QString::compare(m_client->message(), "") == 0) ;
        qDebug() << "Fenêtre Principale : j'ai reçu un message ...";
